...
resource "aws_security_group" "terraformsecuritygroup_3" {
  name        = "mysecuritygrouptf_3"
...
resource "aws_instance" "windows_vm_3" {
  ami           = "ami-01f52dc9cb63c603a"
  instance_type = "t3.xlarge"
  key_name      = "windows_vm"

  associate_public_ip_address = true
  vpc_security_group_ids      = [aws_security_group.terraformsecuritygroup_3.id]
  ...
  depends_on = [aws_security_group.terraformsecuritygroup_3]
}

# # Separate copy function (local-exec provisioner)
# resource "null_resource" "copy_germany_directory" {
#   depends_on = [aws_instance.windows_vm_3]
...
output "instance_public_ip" {
  value = aws_instance.windows_vm_3.public_ip
}

output "instance_id" {
  value = aws_instance.windows_vm_3.id
}
...
output "get_password_command" {
  value = "aws ec2 get-password-data --instance-id ${aws_instance.windows_vm_3.id} --priv-launch-key /home/khaleel/.ssh/windows_vm.pem"
}
