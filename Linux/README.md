...
Internet -> Port 80 -> Internal Network
```

### Network Address Translation (NAT)
NAT modifies the network packets as follows:
- **Source IP Address**: The IP of the sender.
- **Destination IP Address**: The IP of the receiver.
...
When a public server receives a packet, it re-routes it (port forwarding) to an internal server. When a private server sends a packet back, the public server returns it to the original server (Masquerading).

### Configuration Files
To enable packet forwarding, edit the following file (note that modifications may be risky):
- **File**: `/etc/sysctl.d/99-sysctl.conf` or `/etc/sysctl.conf`
...
Here’s a simple end-to-end example of a NAT setup:
1. **Set up DNAT for incoming connections**:
   ```bash
   ...
   # -i enp1s0: incoming interface
   # -s 10.0.0.0/24: source address
   # -p tcp: protocol
   # --dport 8080: destination port
   # -j DNAT: jump to DNAT target
   ```

2. **Set up masquerading for outgoing connections**:
   ```bash
   sudo iptables -t nat -A POSTROUTING -s 10.0.0.0/24 -o enp6s0 -j MASQUERADE
   ```

3. **Test Connectivity**:
   Use the following command to test if NAT is working correctly:
   ```bash
   curl http://<public_ip>:8080/images
   ```

4. **Check NAT Translations**:
   To view the current NAT translations or connections:
   ```bash
   sudo iptables -t nat -L -n -v
   ```

## Firewall and Packet Filtering
Use the Netfilter Framework with `iptables` or `nft`. To set up NAT with `iptables`:
```bash
sudo iptables -t nat -A PREROUTING -i enp1s0 -s 10.0.0.0/24 -p tcp --dport 8080 -j DNAT --to-destination 192.168.0.5:80
```
To masquerade packets:
```bash
sudo iptables -t nat -A POSTROUTING -s 10.0.0.0/24 -o enp6s0 -j MASQUERADE
```

### Viewing Rules and Persisting Changes
To view iptables rules:
```bash
sudo iptables -L --table nat
```
To install persistent iptables:
```bash
...
1. Remove the previous configuration link:
   ```bash
   sudo rm /etc/nginx/sites-enabled/proxy.conf
   ```

2. Create a load balancer configuration:
   - **File**: `/etc/nginx/sites-available/lb.conf`
   ```conf
   upstream mywebserver {
       least_conn;
       server 1.2.3.4 weight=3;
       server 5.6.7.8:8081;
       server 9.10.11.12 down;
...
### NTP Configuration
To manage time synchronization:
```bash
...
### Check Time and Zone
To check the current time and timezone:
```bash
timedatectl
```
...
Configure SSH client settings for specific hosts:
```bash
vim ~/.ssh/config
```
Example:
